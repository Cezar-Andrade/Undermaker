...
if (mode == SOUL_MODE.GRAVITY and gravity_data.jump <= 0 and (other.fragile.duration_time <= 0 or other.fragile.state == 0) and image_angle == other.image_angle and other.image_alpha >= 0.5 and (!gravity_data.prev_platform.found or gravity_data.prev_platform.platform == other)){
	...
	if (other.type == PLATFORM_TYPE.TRAMPOLINE){
		...
		if (other.type == PLATFORM_TYPE.STICKY){
			...
			if (other.anim_timer > 0){
				gravity_data.platform.x -= 2*sin(degtorad(180/10*other.anim_timer))*sin(degtorad(other.image_angle))
				gravity_data.platform.y -= 2*sin(degtorad(180/10*other.anim_timer))*cos(degtorad(other.image_angle))
			}
		}
		var ang = other.image_angle/90
		if (other.type == PLATFORM_TYPE.CONVEYOR){
			if (ang == 0 or ang == 2){
				gravity_data.conveyor_push.val = other.hspeed + other.conveyor_speed*(1 - ang) + 4*((keyboard_check(settings.right[0]) or keyboard_check(settings.right[1])) - (keyboard_check(settings.left[0]) or keyboard_check(settings.left[1])))
			}else{
				gravity_data.conveyor_push.val = other.vspeed + other.conveyor_speed*(2 - ang) + 4*((keyboard_check(settings.down[0]) or keyboard_check(settings.down[1])) - (keyboard_check(settings.up[0]) or keyboard_check(settings.up[1])))
			}
		}else{
			if (ang == 0 or ang == 2){
				gravity_data.conveyor_push.val = other.hspeed + 4*((keyboard_check(settings.right[0]) or keyboard_check(settings.right[1])) - (keyboard_check(settings.left[0]) or keyboard_check(settings.left[1])))
			}else{
				gravity_data.conveyor_push.val = other.vspeed + 4*((keyboard_check(settings.down[0]) or keyboard_check(settings.down[1])) - (keyboard_check(settings.up[0]) or keyboard_check(settings.up[1])))
			}
		}
	...
	if (other.image_angle%180 == 0){
		...
		if (other.type == PLATFORM_TYPE.CONVEYOR){
			gravity_data.platform.vel.x += other.conveyor_speed*dcos(other.image_angle)
		}
	}else{ if (other.image_angle%180 == 90){
		gravity_data.platform.x += other.x - 8*dsin(other.image_angle)
		gravity_data.platform.y += y
		if (other.type == PLATFORM_TYPE.CONVEYOR){
			gravity_data.platform.vel.y -= other.conveyor_speed*dsin(other.image_angle)
		}
	}}
