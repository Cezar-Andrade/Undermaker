...
var _respawn = (type != PLATFORM_TYPE.STICKY and type != PLATFORM_TYPE.TRAMPOLINE)

draw_set_alpha(image_alpha)

if (type == PLATFORM_TYPE.STICKY and anim_timer > 0){
	//Sticky animation of player being hold in place by it, make this a table for multiple players
	var _ = 0
	draw_rectangle((x - 0.5 + 0.5*_ang_sin)*_ang_sin + (obj_player_battle.x - 2)*_ang_cos - 2*dsin(180/10*anim_timer)*max(_ang_sin, 0),
		(y - 0.5 + 0.5*_ang_cos)*_ang_cos - 2*dsin(180/10*anim_timer)*max(_ang_cos, 0) + (obj_player_battle.y - 2)*_ang_sin,
		(x - 0.5 + 0.5*_ang_sin)*_ang_sin + (obj_player_battle.x + 2)*_ang_cos - 2*dsin(180/10*anim_timer)*min(_ang_sin, 0) - 1,
		(y - 0.5 + 0.5*_ang_cos)*_ang_cos - 2*dsin(180/10*anim_timer)*min(_ang_cos, 0) + (obj_player_battle.y + 1)*_ang_sin, false)
	anim_timer++
...
}else if (type == PLATFORM_TYPE.TRAMPOLINE and anim_timer > 0){
...
if (fragile.duration_time <= 0){
		...
		for (var _i=anim_timer - 2; _i<14 + 2*length; _i+=8){
			for (var _j=_i - 2; _j<_i + 2; _j++){
				if (_j < 0){
					...
					draw_point_color(x + (-length/2 + 0.5 + _j)*_ang_cos + (0.5 + _height_offset)*_ang_sin - 0.5, y + (0.5 + _height_offset)*_ang_cos - (-length/2 + _j + 0.5)*_ang_sin - 0.5, _color)
				}else{
					break
				}
			}
	...
	if (draw_get_alpha() > 0){
		if (type == PLATFORM_TYPE.STICKY){
			draw_line(x - length/2*_ang_cos - (0.5 - 0.5*_ang_sin)*_ang_sin + shake[0], y - (0.5 - 0.5*_ang_cos)*_ang_cos - length/2*_ang_sin + shake[1], x + length/2*_ang_cos - (0.5 - 0.5*_ang_sin)*_ang_sin - 1 + shake[0], y - (0.5 - 0.5*_ang_cos)*_ang_cos + (length/2 - 1)*_ang_sin + shake[1])
		}
		for (var i=((type == PLATFORM_TYPE.CONVEYOR) ? anim_timer%8 - 3 : ((type == PLATFORM_TYPE.STICKY) ? length : 0));i<14+2*length;i+=8){
			for (var j=i-2;j<i+3;j++){
				if (j < 0){
					...
					draw_point_colour(x + (-length/2 + 0.5 + j)*_ang_cos + 0.5*_ang_sin - 0.5 + shake[0], y + 0.5*_ang_cos - (-length/2 + j + 0.5)*_ang_sin - 0.5 + shake[1], c_red)
				}else if (j < length + 6){
					draw_point_colour(x + (length/2 - 0.5)*_ang_cos + (j - length + 1.5)*_ang_sin - 1 + shake[0], y + (j - length + 1.5)*_ang_cos - (length/2 - 0.5)*_ang_sin - 0.5 + shake[1], c_red)
				}else if (j < 2*length + 5){
					draw_point_colour(x + (3*length/2 + 5.5 - j)*_ang_cos + 7.5*_ang_sin - 0.5 + shake[0], y + 7.5*_ang_cos - (3*length/2 + 5.5 - j)*_ang_sin - 1 + shake[1], c_red)
				}else if (j < 12 + 2*length){
					draw_point_colour(x + (-length/2 + 0.5)*_ang_cos + (12.5 - j + 2*length)*_ang_sin - 0.5 + shake[0], y + (12.5 - j + 2*length)*_ang_cos + (length/2 - 0.5)*_ang_sin - 0.5 + shake[1], c_red)
				}else{
					break
				}
			}
	...
	var _x1 = x - (length/2 - 0.5)*_ang_cos - 4.5*_ang_sin + shake[0] - 1
	var _y1 = y - 4.5*_ang_cos + (length/2 - 0.5)*_ang_sin + shake[1] - 1
	var _x2 = x + (length/2 - 0.5)*_ang_cos - 4.5*_ang_sin + shake[0] - 1
	var _y2 = y - 4.5*_ang_cos - (length/2 - 0.5)*_ang_sin + shake[1] - 1
		...
		case 0:{
			if (fragile.timer > 0){
				...
				if (fragile.timer == fragile.duration_time + 1){
					//audio_play_sound(snd_platfall2, 0, false) //TODO: Replace sound
...
if (type == PLATFORM_TYPE.TRAMPOLINE and anim_timer > 0){
	...
	var _x1 = x - (_anim_diff + length/2 - 0.5)*_ang_cos - (4.5 + _anim_diff)*_ang_sin - 1
	var _y1 = y - (4.5 + _anim_diff)*_ang_cos + (_anim_diff + length/2 - 0.5)*_ang_sin - 1
	var _x2 = x + (_anim_diff + length/2 - 0.5)*_ang_cos - (4.5 + _anim_diff)*_ang_sin - 1
	var _y2 = y - (4.5 + _anim_diff)*_ang_cos - (_anim_diff + length/2 - 0.5)*_ang_sin - 1
	...
	_x1 = x - (length/2 - 0.5)*_ang_cos + 0.5*_ang_sin - 1
	_y1 = y + 0.5*_ang_cos + (length/2 - 0.5)*_ang_sin - 1
	_x2 = x + (length/2 - 0.5)*_ang_cos + 0.5*_ang_sin - 1
	_y2 = y + 0.5*_ang_cos - (length/2 - 0.5)*_ang_sin - 1
	_x3 = x + (length/2 - 0.5)*_ang_cos + 7.5*_ang_sin - 1
	_y3 = y + 7.5*_ang_cos - (length/2 - 0.5)*_ang_sin - 1
	_x4 = x - (length/2 - 0.5)*_ang_cos + 7.5*_ang_sin - 1
	_y4 = y + 7.5*_ang_cos + (length/2 - 0.5)*_ang_sin - 1
	
	draw_line(_x1, _y1, _x2, _y2)
	draw_line(_x2, _y2, _x3, _y3)
	draw_line(_x4, _y3, _x4, _y4)
	draw_line(_x4, _y4, _x1, _y1)
	...
	anim_timer++
	if (anim_timer == 2){
		//audio_play_sound(snd_bounce, 0, false) //TODO: Replace sound for trampoline bounce
	}else if (anim_timer >= 11){
		anim_timer = 0
	}
}
