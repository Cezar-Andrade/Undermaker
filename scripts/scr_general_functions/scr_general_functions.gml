//OVERWORLD FUNCTIONS - RANDOM ENCOUNTERS
...
function is_random_encounters_active(){
	return obj_game.can_player_encounter_enemies
}
...
function get_encounter_initial_dialog(_enemies){
	...
	if (_length <= 2){
		...
		if (_length == 2){
			switch (_enemies[1]){
				case ENEMY.MAD_DUMMY_DRAWN:{
					_dialog += "[w:30]\nAnother one joins out of jaleously to draw your attention."
				}
				case ENEMY.MAD_DUMMY_SPRITED:{
					_dialog += "[w:30]\nAnother one joins out of envy to sprite them away."
				}
			}
		}
	}else{
...
function get_encounter_functions(_enemies){
	...
	return [BATTLE_START_ANIMATION.NORMAL, undefined, _end_function, 48, 453] //By default these are the values of the start_battle() parameters after the first 2 (except the _end_function one, that is undefined by default)
}
...
function is_overworld_player_moving(_player=obj_player_overworld){
	with (_player){
		return (any_direction_button() and obj_game.state == GAME_STATE.PLAYER_CONTROL)
	}
}

function is_overworld_player_running(_player=obj_player_overworld){
	return (_player.can_run and get_cancel_button() and any_direction_button() and obj_game.state == GAME_STATE.PLAYER_CONTROL)
}
